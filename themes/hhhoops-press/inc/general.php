...
function pl_waiver_columns( $columns ) {
  ...
  $columns['hhhoops_user_name'] = 'User Name';
  $columns['hhhoops_user_email'] = 'User Email';
  return $columns;
}
add_filter( 'manage_nf_sub_posts_columns', 'pl_waiver_columns' );

function pl_waiver_column( $column, $sub_id ) {
  if( $column === 'hhhoops_user_name' || $column === 'hhhoops_user_email' ) {
    //get submission data
    $sub = Ninja_Forms()->form()->get_sub( $sub_id );
    //pull user info
    $user = $sub->get_user();
    //add it in to the column data
    if( $column === 'hhhoops_user_name') echo $user->data->user_nicename;
    if( $column === 'hhhoops_user_email') echo $user->data->user_email;
  }
}
...
function add_apple_touch_icons(){
	...
	echo '<link rel="icon" type="image/png" href="' . get_template_directory_uri() . '/assets/tiny-logos/Hype-Her-Hoops-favicon.png" />' . "\n";
...
if ( ! function_exists( 'theme_setup' ) ) :
    ...
    add_theme_support( 'align-wide' );
		...
		load_theme_textdomain( 'hhhoops-press' );
...
if ( ! function_exists( 'hhhoops_main_nav' ) ) {
	function hhhoops_main_nav() {
    ...
    wp_nav_menu( array(
			...
			'fallback_cb'    => false,
			'walker'         => new hhhoops_Top_Bar_Walker()
		));
	}
}
// Main Navigation Mega
if ( ! function_exists( 'hhhoops_mega_nav' ) ) {
	function hhhoops_mega_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new hhhoops_Mega_Walker()
		));
	}
}
// Main Navigation side Slide
if ( ! function_exists( 'hhhoops_side_slide_nav' ) ) {
	function hhhoops_side_slide_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new hhhoops_Side_Slide_Walker()
		));
	}
}
// Title Nav
if ( ! function_exists( 'hhhoops_title_nav' ) ) {
	function hhhoops_title_nav() {
		wp_nav_menu( array(
			'theme_location' => 'title_nav',
			'container'      => false,
...
// Footer Nav
if ( ! function_exists( 'hhhoops_footer_nav' ) ) {
	function hhhoops_footer_nav() {
		wp_nav_menu( array(
...
if ( ! function_exists( 'hhhoops_excerpt' ) ) :
	/**
	 * Displays the optional excerpt.
	 * Wraps the excerpt in a div element.
	 * Create your own hhhoops_excerpt() function to override in a child theme.
	 * @param string $class Optional. Class string of the div element. Defaults to 'entry-summary'.
	 */
	function hhhoops_excerpt( $class = 'entry-summary' ) {
...
if ( ! function_exists( 'hhhoops_excerpt_more' ) && ! is_admin() ) :
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a 'Continue reading' link.
 * Create your own hhhoops_excerpt_more() function to override in a child theme.
 * @return string 'Continue reading' link prepended with an ellipsis.
 */
function hhhoops_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'hhhoops-press' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'hhhoops_excerpt_more' );
endif;

if ( ! function_exists( 'hhhoops_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 * Create your own hhhoops_entry_meta() function to override in a child theme.
 */
function hhhoops_entry_meta() {
	if ( 'posts' === get_post_type() ) {
		$author_avatar_size = apply_filters( 'hhhoops_author_avatar_size', 49 );
		printf( '<span class="byline"><span class="author vcard">%1$s <span class="screen-reader-text">%2$s </span> <a class="url fn n" href="%3$s">%4$s</a></span></span>',
			get_avatar( get_the_author_meta( 'user_email' ), $author_avatar_size ),
			_x( 'Author', 'Used before post author name.', 'hhhoops-press' ),
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'hhhoops-press' ) ),
	...
	if ( 'posts' === get_post_type() ) {
// 		hhhoops_entry_taxonomies();
	}

	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'hhhoops-press' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'hhhoops_entry_date' ) ) :
/**
 * Prints HTML with date information for current post.
 * Create your own hhhoops_entry_date() function to override in a child theme.
 */
function hhhoops_entry_date() {
...
// 		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time> | <time class="updated" datetime="%3$s">%4$s</time>';
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'hhhoops-press' ),
...
function hhhoops_admin_menu() {
	add_menu_page( 'OGP Data Admin', 'Data', 'manage_options', 'hhhoops_data', 'hhhoops_admin', 'dashicons-chart-line', 26  );
// 	add_submenu_page( 'hhhoops_data', 'hhhoops Data Dashboard', 'Dashboard', 'manage_options', 'hhhoops_data', 'hhhoops_admin' );
}
add_action( 'admin_menu', 'hhhoops_admin_menu' );

//build admin page
function hhhoops_admin(){
	wp_enqueue_style( 'hhhoops_admin_styles', get_stylesheet_directory_uri() . '/css/hhhoops-admin-style.css' );
	$g_action = filter_input( INPUT_GET, 'g_action', FILTER_SANITIZE_URL );
	$g_action = ( empty($g_action) ) ? 'settings' : $g_action;
	$hhhoops_sections = array(
		'settings' => array(
			'title'		=> 'Manage Site Settings',
			'tab_name'	=> 'Settings',
			'url'			=> '?page=hhhoops_data&g_action=settings'
		)
	);
	echo '<div class="hhhoops_data_manager_wrapper">';
	$hhhoops_admin_url = '';
	$nav = '';
	foreach( $hhhoops_sections as $section => $section_data ) {
		if( $section == $g_action ) {
			echo '<h1>' . $section_data['title'] . '</h1>';
			$hhhoops_admin_url = $section_data['url'];
	...
	echo '<nav class="nav-tab-wrapper">' . $nav . '</nav><div class="hhhoops_data_manager_content_wrapper">';
	switch( $g_action ) {
		case 'export_data':
			hhhoops_export();
			break;
		case 'record_data':
			hhhoops_record_stats(); 
			break;
		case 'settings':
			$hhhoops_admin_settings = array(
				'display' => array(
			...
			function hhhoops_update_admin_site_settings( $old, $new ) {
				foreach( $new as $item => $item_data ) {
					foreach( $item_data as $item_name => $item_value ) {
						$old[$item]['items'][$item_name]['value'] = hhhoops_process_data_point( $old[$item]['items'][$item_name]['type'], $item_value);
            if( $old[$item]['items'][$item_name]['value'] === null || $old[$item]['items'][$item_name]['value'] === 'null' ) $old[$item]['items'][$item_name]['value'] = '';
					}
				}
				return $old;
			}
			foreach( $hhhoops_admin_settings as $setting_set => &$setting_data ) {
				if( $_POST['g_process'] == 'process' ) {
					$hhhoops_db_option = $_POST['hhhoops_admin_form_data'][$setting_set];
					$update = update_option( 'hhhoops_' . $setting_set, $hhhoops_db_option );
					$setting_data['section_records'] = hhhoops_update_admin_site_settings( $setting_data['section_records'], $hhhoops_db_option );
					if( $update === true ) echo '<p class="success">Updated settings data!</p>';
				} else {
					$hhhoops_db_option = get_option( 'hhhoops_' . $setting_set );
					if( $hhhoops_db_option !== false && !empty($hhhoops_db_option) ) $setting_data['section_records'] = hhhoops_update_admin_site_settings( $setting_data['section_records'], $hhhoops_db_option );
        }
			} ?>
			<h3>General Site Settings</h3>
			<form id="hhhoops_form" method="post" class="hhhoops_form" action="<?php echo $hhhoops_admin_url; ?>">
				<input type="hidden" name="g_process" value="process" />
			<?php foreach( $hhhoops_admin_settings as $setting_set_re => $setting_data_re ) : ?>
				...
				<table class="hhhoops_form_section form-table">
					<tbody>
					<?php foreach( $record_data['items'] as $element => $element_data ) {
						$element_data['tag'] = 'hhhoops_admin_form_data[' . $setting_set_re . '][' . $record . '][' . $element . ']';
						$element_data['description'] = ( empty($element_data['description']) ) ? '' : '<small>' . $element_data['description'] . '</small>';
						echo hhhoops_template_construction( $element_data );
					} ?>
					</tbody>
				</table>
				<?php endforeach;
...
function g365_data_receiver(){
  $grassroots_keys = get_option( 'hhhoops_g365_connector' );
...
function send_html_email($email, $subject, $message) {
  $headers = ( strpos( site_url(), 'dev' ) !== false ) ? array('From: hhhoops Dev Customer Service <no-reply@dev.elitebasketballcircuit.com>') : array('From: hhhoops Customer Service <no-reply@elitebasketballcircuit.com>');
...
// 	$pageID = ( $pageID === null ) ? $post->ID : $pageID;
	...
	$hhhoops_site_ads = get_option( 'hhhoops_display' );
	//if the page doesn't have an ad or the site global is empty, don't add to array
	$ad_info['go'] = ( empty($hhhoops_page_ad['link']) || empty($hhhoops_page_ad['img']) || empty($hhhoops_site_ads) ) ? false : true;
	...
	if ( $ad_info['go'] ) {
		...
		$hhhoops_site_ads['site_1']['element_type'] = 'rotator';
		$hhhoops_site_ads['site_2']['element_type'] = 'rotator';
		//build rotator element for page ad
		$ad_info['ad_content'] .= hhhoops_template_construction( $hhhoops_page_ad );
		//if we have a global ad, build and add it
		if( !empty($hhhoops_site_ads['site_1']['img']) ) $ad_info['ad_content'] .= hhhoops_template_construction( $hhhoops_site_ads['site_1'] );
		//if we have a secondary global ad, build if and add it along with another primary ad to help the flow
		if( !empty($hhhoops_site_ads['site_2']['img']) ) {
			$ad_info['ad_content'] .= hhhoops_template_construction( $hhhoops_page_ad );
			$ad_info['ad_content'] .= hhhoops_template_construction( $hhhoops_site_ads['site_2'] );
		}
	}
	//on the front page, and if we have a global splash setting
	if( is_front_page() && !empty($hhhoops_site_ads['site_3']) ) {
		//controls for the splash
		$hhhoops_splash = array(
			'title'				=> ( empty($hhhoops_site_ads['site_3']['title']) ) ? '' : $hhhoops_site_ads['site_3']['title'],
			'link'				=> ( empty($hhhoops_site_ads['site_3']['link']) ) ? '' : $hhhoops_site_ads['site_3']['link'],
			'img'					=> ( empty($hhhoops_site_ads['site_3']['img']) ) ? '' : $hhhoops_site_ads['site_3']['img'],
			'img_mobile'	=> ( empty($hhhoops_site_ads['site_3']['img_mobile']) ) ? '' : $hhhoops_site_ads['site_3']['img_mobile'],
			'description'	=> ( empty($hhhoops_site_ads['site_3']['description']) ) ? '' : $hhhoops_site_ads['site_3']['description']
		);
		//if we have minimum info process the splash
		if( !empty($hhhoops_splash['title']) && !empty($hhhoops_splash['link']) && !empty($hhhoops_splash['img']) ) {
			//build the splash html
			$ad_info['splash'] = '<div class="reveal text-center" id="hhhoops_home_reveal" aria-labelledby="' . $hhhoops_splash['title'] . '" data-reveal><div class="relative"><h1 id="reveal-title" class="show-for-sr">' . $hhhoops_splash['title'] . '</h1>';
			//if there is a derscription, add it
			if( !empty($hhhoops_splash['description']) ) $ad_info['splash'] .= '<p class="reveal-description">'. $hhhoops_splash['description'] . '</p>';
			//if there is a mobile image, add it
			if( !empty($hhhoops_splash['img_mobile']) ) $ad_info['splash'] .= '<p class="text-center show-for-small-only"><a href="' . $hhhoops_splash['link'] . '"><img src="'. $hhhoops_splash['img_mobile'] . '" alt="'. $hhhoops_splash['title'] . '" title="'. $hhhoops_splash['title'] . '" /></a></p>';
			//if there is a mobile image, add supporting class to the large image
			$mobile_img_class = ( empty($hhhoops_splash['img_mobile']) ) ? '' : ' show-for-medium';
			//add main splash image
			$ad_info['splash'] .= '<p class="text-center' . $mobile_img_class . '"><a href="' . $hhhoops_splash['link'] . '"><img src="'. $hhhoops_splash['img'] . '" alt="'. $hhhoops_splash['title'] . '" title="'. $hhhoops_splash['title'] . '" /></a></p>';
...
function hhhoops_template_construction( $ele_data ) {
	$hhhoops_admin_form_elements = array(
		'input' 	=> array(
			'template' => '<tr>
					<th>
	...
	$ele = $hhhoops_admin_form_elements[$ele_data['element_type']]['template'];
	foreach( $hhhoops_admin_form_elements[$ele_data['element_type']]['vars'] as $dex => $var ) {
    if( $var === 'options' ) {
      $all_options = '';
      $option_ele = $hhhoops_admin_form_elements[$ele_data['element_type']]['template_option'];
...
function hhhoops_process_data_point( $name, $point ) {
...
function hhhoops_build_dates($dates, $type = 1, $abbv = false, $add_reg = false) {
    ...
    switch( $type ){
      ...
      case 'date':
        ...
        if( empty($event_object->dates) ) {
          ...
          $date_reg = hhhoops_build_dates($event_object->dates, 2, false, $reg_days);
          if( $reg_days === false ) {
      ...
      case 'full_address':
        if( empty($event_object->short_locations) ) {
          ...
          $reg_days = $atts['reg_days'];
          $date_reg = hhhoops_build_dates($event_object->short_locations, 2, false, $reg_days);
          if( $reg_days === false ) {
            $data_compile .= $date_reg;
          } else {
            $data_compile .= $date_reg[0];
