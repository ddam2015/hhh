...
// 	End add multiple products
  ...
  //woocommerce wrappers
	...
	function hhhoops_wrapper_start() {
		echo '<section id="content" class="grid-x site-main woocomm-wrap" role="main">';
		echo '<div class="cell small-12">';
	}
	function hhhoops_wrapper_end() {
		echo '</div>';
		echo '</section>';
	}
	add_action('woocommerce_before_main_content', 'hhhoops_wrapper_start', 10);
	add_action('woocommerce_after_main_content', 'hhhoops_wrapper_end', 10);
	...
	function hhhoops_woocommerce_image_dimensions() {
		global $pagenow;
	...
	add_action( 'after_switch_theme', 'hhhoops_woocommerce_image_dimensions', 1 );

	//remove the breadcrumb (on product pages)
	function hhhoops_remove_wc_breadcrumbs() {
			remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
	}
	add_action( 'init', 'hhhoops_remove_wc_breadcrumbs' );
  ...
  add_action( 'woocommerce_before_cart', 'woocommerce_checkout_login_form' );
	...
	function hhhoops_place_order_button_text() { return __( 'Complete Registration', 'woocommerce' ); }
	add_filter( 'woocommerce_order_button_text', 'hhhoops_place_order_button_text' );
  ...
  //change add to cart button text
	function hhhoops_woocommerce_product_add_to_cart_text(){
		return 'Register Now';
	}
	add_filter( 'woocommerce_product_add_to_cart_text' , 'hhhoops_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'hhhoops_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_booking_single_add_to_cart_text', 'hhhoops_woocommerce_product_add_to_cart_text' );
...
function hhhoops_wc_scripts_styles() {
	if( is_admin() ) return;
  wp_dequeue_style( 'stripe_styles' );
  // js helpers for the cart page
  if( is_cart() || is_checkout() ) {
    wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('jquery'), '695450', true );
  }
}
add_action( 'wp_enqueue_scripts', 'hhhoops_wc_scripts_styles', 100);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/hhhoops-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
...
function hhhoops_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $extra_types = array( 'fee', 'gear', 'event', 'team-info' );
            ...
            case 'event':
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                $extra_fields[] = array(
                  ...
                  'class'       => array($product_cat_slug, 'woo-registration__input', 'inputLong','team-name')
                );
                $extra_fields[] = array(
                  'title'       => $product_name,
                  'type'        => 'select',
                  ...
                  'options'     => array(
                            ''    => 'Requested Division',
                            'Girls 4th Grade'  => "Girls 4th Grade",
                            'Girls 5th Grade'  => "Girls 5th Grade",
                            'Girls 6th Grade'  => "Girls 6th Grade",
                            'Girls 7th Grade'  => "Girls 7th Grade",
                            'Girls 8th Grade'  => "Girls 8th Grade",
                            '17U' => "17U",
                            '16U' => "16U",
                            '15U'  => "15U",
//                             'Scholastic_Frosh/Soph' => "Scholastic Frosh/Soph",
//                             'Scholastic_JV' => "Scholastic JV",
//                             'Scholastic_Varsity'  => "Scholastic Varsity",
                  )
                );
                $extra_fields[] = array(
                  'title'       => $product_name,
...
add_filter( 'woocommerce_before_cart' , 'hhhoops_cart_setup');
function hhhoops_cart_setup() {
  //load in global proc var
  global $cart_parse;
  //process cart items
  $cart_parse = hhhoops_cart_process();
  ...
  if(!empty($cart_parse[2]) ){
    ...
    add_filter( 'woocommerce_after_order_notes' , function( $fields ) use ( $order_data_field ) { return hhhoops_player_checkout_field( $field, $order_data_field ); });
  }
}
...
function hhhoops_player_checkout_field( $checkout, $extra_fields = null ) {
  ...
  echo '<input type="hidden" name="hhhoops_extra_data_required" value="' . implode(',',$added_field_names_required) . '" />';
  echo '<input type="hidden" name="hhhoops_extra_data" value="' . implode(',',$added_field_names) . '" />';
}

//make the extra data required based on the field names variable
add_action('woocommerce_checkout_process', 'hhhoops_player_checkout_field_process');
function hhhoops_player_checkout_field_process() {
  // Check if we have extra data
  if ( $_POST['hhhoops_extra_data_required'] ) {
    //parse and loop through the set to make sure we have all the fields filled out
    $fields = explode(',', $_POST['hhhoops_extra_data_required']);
...
add_action( 'woocommerce_checkout_update_order_meta', 'hhhoops_player_checkout_field_update_order_meta' );
function hhhoops_player_checkout_field_update_order_meta( $order_id ) {
  //join the extra_data list if we need to
  $hhhoops_extra_data = array();
  $hhhoops_extra_data_fields = array();
  if ( $_POST['hhhoops_extra_data'] ) {
    $hhhoops_extra_data[] = $_POST['hhhoops_extra_data'];
    $hhhoops_extra_data_fields[] = $_POST['hhhoops_extra_data'];
  }
  if ( $_POST['hhhoops_extra_data_required'] ) {
    $hhhoops_extra_data[] = $_POST['hhhoops_extra_data_required'];
    $hhhoops_extra_data_fields[] = $_POST['hhhoops_extra_data_required'];
  }
  //save the field reference so we know what to pull later
  if ( !empty($hhhoops_extra_data) ) update_post_meta( $order_id, 'hhhoops_extra_data', sanitize_text_field( implode(',', $hhhoops_extra_data) ) );
  // Check if we have extra data
  if ( !empty($hhhoops_extra_data_fields) ) {
    //parse extra field names, then loop and save
    $fields = explode(',', implode(',', $hhhoops_extra_data_fields));
    foreach( $fields as $dex => $key ) {
...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'hhhoops_player_checkout_field_display_admin_order_meta', 10, 1 );
function hhhoops_player_checkout_field_display_admin_order_meta($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'hhhoops_extra_data', true );
  if( !empty($fields) ) {
...
function hhhoops_edit_memberships_actions( $actions ) {
...
add_filter( 'wc_memberships_members_area_my-memberships_actions', 'hhhoops_edit_memberships_actions' );
add_filter( 'wc_memberships_members_area_my-membership-details_actions', 'hhhoops_edit_memberships_actions' );
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      if( !empty($series_events[ $product_id ][ 'data_pull' ]) ) {
        ...
        if( empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) {
          ...
          //add to the series order so we can be organized
          $series_order[ $product_id ] = date( 'Y-m-d', strtotime(hhhoops_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 4)));
          ...
          if( $payment_options ) {
            ...
            $series_events[ $product_id ][ 'label' ][] = hhhoops_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
...
add_filter( 'woocommerce_checkout_fields' , 'hhhoops_modify_checkout_fields' );
// Our hooked in function - $fields is passed via the filter!
function hhhoops_modify_checkout_fields( $fields ) {
   unset($fields['order']['order_comments']);
   return $fields;
}

//validate the checkout form before finishing the process of writting data to hhhoops about the purchase
add_action('woocommerce_after_checkout_validation', 'process_hhhoops_data');
function process_hhhoops_data($form_data) {
...
add_action( 'woocommerce_checkout_update_order_meta', 'hhhoops_update_meta_form_fields' );
function hhhoops_update_meta_form_fields( $order_id ) {
  if ( ! empty( $_POST['order_data'] ) ) update_post_meta( $order_id, '_order_data', $_POST['order_data'] );
}

//display and custom order data for admin
add_action( 'woocommerce_admin_order_data_after_shipping_address', 'hhhoops_checkout_field_display_admin_order_meta', 10, 1 );
function hhhoops_checkout_field_display_admin_order_meta($order){
...
// function OLD_hhhoops_checkout_field_display_admin_order_meta($order){
//   //for setting form restrictions/helpers
//   $hhhoops_cart_items = array();
//   //return string
//   $to_write = '';
...
//       // update functionality
//       //
//   wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/hhhoops-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
// 	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
//   wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );
...
// NEW
function get_stock_status_text( $product, $name, $term_slug ) {
    // Check if product belongs to category with ID 27 (assuming 27 is the gear category ID)
    $categories = $product->get_category_ids();
    if (in_array(27, $categories)) {
        return ''; // Return empty string for products in "gear" category
    }

    // For products not in "gear" category
    $in_stock = ''; // Initialize $in_stock variable outside the loop
    foreach ( $product->get_available_variations() as $variation ) {
        if ($variation['attributes'][$name] == $term_slug ) {
            $in_stock = ( $variation['is_in_stock'] == 1 ) ? '' : ' - SOLD OUT';
            $in_stock = ( $in_stock === '' ) ? ( ( is_integer($variation['max_qty']) ) ? ' - ' . $variation['max_qty'] . ' spots left' : '' ) : $in_stock;
            break; // Break only if matching variation found
        }
    }
    return $in_stock;
}
...
add_action( 'show_user_profile', 'hhhoops_user_fields_display' );
add_action( 'edit_user_profile', 'hhhoops_user_fields_display' );

function hhhoops_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/hhhoops-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '695450', true );
  ?>
	<h3><?php esc_html_e( 'Additional User Data', 'hhhoops' ); ?></h3>
  <div class="options_group g365_manage_wrapper">
...
add_action( 'user_profile_update_errors', 'hhhoops_user_field_error', 10, 3 );
function hhhoops_user_field_error( $errors, $update, $user ) {
	if ( ! $update ) {
		return;
	}
}

add_action( 'personal_options_update', 'hhhoops_user_fields_update' );
add_action( 'edit_user_profile_update', 'hhhoops_user_fields_update' );

function hhhoops_user_fields_update( $user_id ) {
...
?>
<?php
// Refresh page on quantity changes
